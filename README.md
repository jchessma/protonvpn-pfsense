# protonvpn-pfsense
Scripts to scrape ProtonVPN's VPN status page for a region, identify the fastest server, ensure it supports P2P connections, and send the IP to a pfSense server.
...
Requirements are pretty limited. Mainly you need a ProtonVPN account, SSH access to the pfSense server, and the excellent pfSense REST API from jaredhendrickson31 (https://github.com/jaredhendrickson13/pfsense-api) installed on your pfSense box.

There are two separate scripts here, one to get the desired server IP address and one to update the pfSense firewall:
 * scraper.py - Python script to scrape the ProtonVPN downloads page screen, identify the best server, ensure it supports P2P, find the IP, and store the IP in a temporary file.
 * config.json - Configuration file (JSON format) containing the username, password, mailbox password, TOTP secret key, and other configuration details.
...
For the Python script to work you need to ensure the appropriate Python 3 libraries are installed (pip3 install ...) and update the script as follows:
